...
- **제네릭 아이콘 회피**: 도메인 객체는 컬러 타일 없이 중립 모노크롬 아이콘 + 타이포로 식별. lucide stroke는 1.75로 가늘게.
...
**식별 (Identity)**: 색은 절제한다. 도메인 객체는 **중립 모노크롬 아이콘 + 강한 타이포**로 식별하고, 컬러 타일/스퀘어클은 쓰지 않는다(미니멀 우선).
