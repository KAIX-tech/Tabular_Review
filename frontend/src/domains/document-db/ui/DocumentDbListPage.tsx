  ...
  return (
    <div className="h-screen overflow-y-auto">
      <header className="h-16 px-8 flex items-center justify-between border-b border-border bg-surface">
        ...
        <button type="button" className="tr-btn tr-btn-primary h-9 px-3.5">
          <Plus className="w-4 h-4" strokeWidth={2} />새 Document DB
        </button>
      </header>
      ...
      <div className="px-8 py-7 max-w-6xl">
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-36 tr-card animate-pulse" />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {documentDbs.map((db) => (
              <div
                ...
                className="group relative tr-card p-5 cursor-pointer transition-[box-shadow,border-color,transform] duration-150 hover:shadow-card hover:border-border-strong hover:-translate-y-0.5"
              >
                <button
                  type="button"
                  title="더보기"
                ...
                <div className="flex items-center gap-3.5">
                  <DbSquircle id={db.id} name={db.name} className="w-11 h-11 rounded-xl text-base" />
                  <div className="min-w-0 pr-6">
                    <h2 className="font-semibold text-ink truncate leading-tight">{db.name}</h2>
                    {db.description && <p className="text-xs text-ink-3 truncate mt-1">{db.description}</p>}
                  </div>
                </div>

                <div className="mt-5 pt-4 border-t border-border flex items-center justify-between text-xs text-ink-3">
                  <span className="flex items-center gap-1.5">
                    <span className="tabular-nums text-ink-2 font-medium">{db.documentCount}</span> 문서
                    <span className="text-border-strong mx-1">·</span>
                    <span className="tabular-nums text-ink-2 font-medium">{db.columnCount}</span> 컬럼
                  </span>
