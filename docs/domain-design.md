...
#### 에이전트 루프 (provider-portable JSON 도구 루프)

native function-calling 대신 **JSON 루프**로 추상화 — `TextGenerationPort.generate_json`을
그대로 재사용하므로 Gemini(dev)·온프렘 GLM 양쪽에서 모델의 function-calling 지원 없이 동작.

```text
system: 도구 스펙 + 규칙 제시
loop (≤ MAX_STEPS):
  LLM → JSON 반환:
     {"action": "<tool>", "args": {…}}   ─▶ 서버가 도구 실행 → 결과를 다음 턴에 주입
     {"answer": "…", "sources": [{kind:'chunk'|'cell', id, quote, …}]}  ─▶ 종료
```
