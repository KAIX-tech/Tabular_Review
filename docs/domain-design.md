...
- 임베딩/리랭커는 `llm`과 별개 추론 엔드포인트 → 포트 `EmbeddingPort`/`RerankPort`로 추상화.
  - **구현 상태**: `AI_PROVIDER=onprem` 임베딩 = **HF TEI**(`POST /embed`) 어댑터 `TeiEmbedder`(`EMBEDDING_BASE_URL`), 생성 = **vLLM** OpenAI 호환 `VllmTextGeneration`(`VLLM_*`, `response_format=json_object`, `VLLM_JSON_OBJECT_MODE`로 토글). 리랭커(`RerankPort`)는 **미구현(추후)** — 현재 폴백은 벡터 top-n 직행.
- **dev(Gemini)는 재정렬(②) 생략** — 벡터 검색 top-n을 바로 사용(폴백 자체가 드물게 발동).
  `RerankPort`는 온프렘에서만 구현 예정(미설정 시 no-op = 벡터 순위 유지).
...
**오케스트레이션 = LangGraph**, **도구 호출 = 모델 native tool-calling**(GLM/vLLM·Gemini 모두
지원하도록 구성됨). LangChain chat model — `langchain-openai`(ChatOpenAI→vLLM) /
`langchain-google-genai`(Gemini) — 에 도구를 `bind_tools`하고, LangGraph 프리빌트
tool-calling 패턴(`create_react_agent` 또는 model 노드 + `ToolNode` + `tools_condition`)으로 돌린다.
LangGraph/LangChain은 **`chat`의 application/infrastructure 레이어에만** 둔다(도메인 순수: 프레임워크
import 금지). 의존: `langgraph`, `langchain-core`, `langchain-openai`, `langchain-google-genai`
(+ Langfuse LangChain 콜백). **추출(extraction)은 기존 `generate_json` 어댑터를 그대로 유지** —
챗만 LangChain chat model 경로를 쓴다(두 경로 공존).

```text
graph: model(bind_tools) ──tool_calls──▶ ToolNode(toolset 실행) ──results──▶ model ⟳(≤MAX_STEPS)
                          └──no tool────▶ finalize(출처 매핑 + 영속)
```

- **상한**: `MAX_STEPS`(설정/recursion_limit)로 무한 루프 방지. 초과 시 마지막 근거로 best-effort.
- **출처**: 도구 결과가 **이미 `chunk_id`/`cell_id`를 포함**하므로(§2.13 카탈로그), 에이전트가
  답변 근거로 사용한 청크/셀을 **id로 정확 매핑**해 ChatSource로 저장 — 추출처럼 quote를 청크에
  되맞추는 fuzzy 매칭이 **불필요**(정확도↑). (모델이 명시 인용하면 그 id 우선, 아니면 사용된 도구
  결과 순으로 rank)
...
14. **도구 호출 방식** → ✅ **모델 native tool-calling.** 온프렘 GLM(vLLM)이 tool-calling으로
   구성돼 있고 Gemini도 지원하므로, JSON 루프 대신 native tool-calling을 쓴다(견고성·정확도↑).
   LangChain chat model(`langchain-openai`→vLLM / `langchain-google-genai`→Gemini)에 `bind_tools`.
   **챗만** LangChain chat model 경로, **추출은 기존 `generate_json` 유지**(두 경로 공존).
...
17. **에이전트 오케스트레이션 프레임워크** → ✅ **LangGraph + native tool-calling**(프리빌트
   `create_react_agent` 또는 model+`ToolNode`+`tools_condition`). LangGraph/LangChain은 `chat`
   application/infra 레이어에만, 도메인은 포트만 의존. 스트리밍(`astream`)→SSE 스텝. 트레이싱은
   **Langfuse LangChain 콜백**. 의존: `langgraph`, `langchain-core`, `langchain-openai`,
