...
| `query_cells` | `documentDbId, columns?, filter?` | 추출 그리드 행×열 값(+cellId) | extraction |
...
> 비고: 도구는 모두 **읽기 전용**(부작용 없음). MAX_STEPS·도구 결과 토큰 상한·DB 스코프는
> 튜닝/안전 파라미터(설정값). 리랭커·정형 필터 고도화는 후속(§10).
...
1. **최우선 — 운영 LLM E2E 검증.** 운영 pull 후 대표 질의 2종 스모크(phase-4 §6) +
   출처 칩/스텝 타임라인 실답변 확인.
2. (후속) 챗 외 UI 백로그(아래 3). — 출처 점프/참조 패널 통일은 완료(원문 = md 뷰어
   하이라이트, 추출값 = 공용 셀 디테일 카드 + `?cell=` 딥링크).
