...
| `get_document` | `documentId` | 문서 markdown(또는 슬라이스) | ingestion |
...
- **출처**: 도구 결과가 **이미 `chunk_id`/`cell_id`를 포함**하므로(§2.13 카탈로그), 실행 후 최종
  ...
  quote를 청크에 되맞추는 fuzzy 매칭이 **불필요**(정확도↑). (모델이 명시 인용하면 그 id 우선,
  아니면 사용된 도구 결과 순으로 rank)
...
| POST | `/chat/sessions/{sid}/messages` | 질문 전송 → **에이전트 실행**, **SSE**로 `step`(도구 호출) → `answer`(답변+출처) → `done` 스트림(§2.13) |
