...
- **ChatSession / ChatMessage** = 전체(또는 특정) DB에 대한 대화. 챗은 단발 RAG가 아니라
  **에이전트(Agentic Search)** 로 동작 — 카탈로그(DB→컬럼→문서/셀/청크)를 도구로 탐색해
  답한다(§2.13). 답변의 출처는 **청크(비정형) 또는 셀(정형)** 을 가리킨다.
- **User** = 소유/감사 주체. (인증 화면은 범위 외지만 모델에는 둔다.)

> 핵심 결정: **grounding은 같은 행렬을 공유한다.** 추출(Cell)은 `DocumentChunk`를 인용하고,
> 챗(Message)의 출처는 **청크 또는 셀**을 인용한다 — "셀 근거 보기"와 "챗 출처 보기"가
> 동일한 점프 메커니즘을 공유한다.
...
세션 내 메시지. assistant 메시지는 출처(ChatSource)를 갖고, 에이전트가 거친 **도구 호출
과정(steps)** 을 보존한다(투명성·재현). (사용자 스케치의 `Chat`)
...
| `content` | text | 최종 답변(assistant) / 질문(user) |
| `steps` | jsonb null | assistant 메시지의 **에이전트 도구 호출 추적**: `[{step, tool, args, summary}]`. SSE로 라이브 스트림하고 완료 시 저장 → 세션 재오픈 시 "사고 과정" 재현(§2.13). |
| `created_at` | timestamptz | (프론트 `timestamp` 대응) |

### 2.11 ChatSource  *(chat)*

챗 답변의 출처. **청크(비정형) 또는 셀(정형)** 중 하나를 가리킨다 — 에이전트가 `search_chunks`로
찾은 원문 스니펫이면 청크를, `query_cells`로 비교한 추출값이면 셀을 인용한다. documentDb/
documentName 등 표시 메타는 저장하지 않고 조인/캐시로 채운다(프론트 `ChatSource`와 정합).

| 필드 | 타입 | 비고 |
|---|---|---|
| `id` | uuid (PK) | |
| `message_id` | uuid → ChatMessage (FK, cascade) | |
| `chunk_id` | uuid → DocumentChunk null | 비정형 출처(청크 인용) |
| `cell_id` | uuid → Cell null | 정형 출처(추출 셀 값 인용). `chunk_id`와 **둘 중 하나**가 채워진다 |
| `quote` | text | 인용 스니펫 또는 셀 값 |
| `page` | int null | (청크 출처일 때) |
| `rank` | int | 출처 표시 순번(①②③) |
| `created_at` | timestamptz | |
...
| GLM (dev, OpenRouter) | 모델별(예: GLM-4.6 ~200k) | dev에서도 대부분 [A]로 처리됨 |
...
추출 폴백([B])과 챗 에이전트의 `search_chunks` 도구(§2.13)가 공유하는 **2단계 retrieve → rerank**
(챗은 이 청크 검색을 *여러 도구 중 하나*로 호출한다):
...
- **모델 구성**: 임베딩 = **BGE-M3**(1024d, HF TEI), 리랭커 = **BGE-Reranker-V2-M3**(추후),
  생성 = **GLM**(prod=온프렘 vLLM, dev=OpenRouter — 둘 다 OpenAI 호환).
- 임베딩/리랭커는 `llm`과 별개 추론 엔드포인트 → 포트 `EmbeddingPort`/`RerankPort`로 추상화.
  - **구현 상태**: 임베딩 = **HF TEI**(`POST /embed`) 어댑터 `TeiEmbedder`(`EMBEDDING_BASE_URL`, 단일 어댑터), 생성 = OpenAI 호환 `VllmTextGeneration`(`LLM_PROVIDER`로 vLLM/OpenRouter 타깃 선택, `response_format=json_object`, `VLLM_JSON_OBJECT_MODE`로 토글). 리랭커(`RerankPort`)는 **미구현(추후)** — 현재 폴백은 벡터 top-n 직행.
- **재정렬(②)은 현재 전 환경에서 생략** — 벡터 검색 top-n을 바로 사용(리랭커 미구현, 폴백 자체도 드물게 발동).
  `RerankPort` 구현 시 온프렘에서 활성화(미설정 = no-op = 벡터 순위 유지).
...
### 2.13 챗 — Agentic Search

> **원칙: 챗은 단발 RAG가 아니라 카탈로그를 탐색하는 에이전트다.** 유저 질문에 답하려고
> LLM이 **읽기 전용 도구**를 단계적으로 호출한다 — 먼저 어떤 **Document DB**가 적합한지 찾고,
> 그 DB의 **컬럼(추출 스키마)** 을 확인한 뒤, 적절한 **문서·셀·청크**를 조회한다. 이로써
> 정형 질의(추출 그리드 값 비교)와 비정형 질의(원문 시맨틱 검색)를 한 흐름에서 처리한다.

#### 도구 카탈로그 (read-only — 기존 컨텍스트의 read 포트 위)

| 도구 | 입력 | 반환 | 컨텍스트 |
|---|---|---|---|
| `list_document_dbs` | — | `[{id,name,description,columnCount,documentCount}]` | document_db |
| `list_columns` | `documentDbId` | `[{id,name,type,prompt}]` (추출 스키마) | document_db |
| `list_documents` | `documentDbId` | `[{id,name,status}]` | ingestion |
| `find_documents` | `keyword, documentDbId?` | 문서명 키워드 검색(ILIKE AND, 전 DB 횡단, 상한 20+truncated) — phase-5 D12 | ingestion |
| `query_cells` | `documentDbId, columnIds?, filters?, sort?, limit?, countOnly?` | 추출 그리드 행×열 값(+cellId). `filters=[{columnId,op,value}]`(op v1: eq/ne/contains/in/is_empty, AND 결합)는 **서버가 파라미터라이즈드 SQL로 컴파일**(모델 raw SQL 금지), `countOnly`는 행 덤프 없이 집계 — phase-5 D13 | extraction |
| `search_chunks` | `query, documentDbId?/documentId?, k?` | `[{chunkId,documentId,documentName,page,quote}]` | ingestion(§2.12 검색) |
| `get_document` | `documentId, offset?, length?` | 문서 markdown **슬라이스**(전문 반환 금지 — 도구 결과 토큰 상한, phase-4 D11) | ingestion |

예) "준거법=대한민국 계약 중 계약기간이 가장 긴 것?" → `list_document_dbs` → "계약서" 선택
→ `list_columns`(준거법·계약기간 확인) → `query_cells` → 비교/추론 → 셀 출처와 함께 답변.

#### 에이전트 루프 (LangGraph + native tool-calling)

**오케스트레이션 = LangGraph 런타임**, **도구 호출 = 모델 native tool-calling**(GLM이 tool-calling
구성됨). LangChain v1(2025-10 GA) 기준 정석은 **`langchain.agents.create_agent`** — LangGraph 런타임
위에 빌드된 ReAct 에이전트로, 도구 바인딩·루프·체크포인트·스트리밍을 한 번에 제공한다. 모델은
`langchain-openai`의 `ChatOpenAI`를 주입(`LLM_PROVIDER`로 vLLM/GLM(prod) 또는 OpenRouter GLM(dev)
타깃 선택 — 둘 다 OpenAI 호환). (구버전 `langgraph.prebuilt`의 `create_react_agent`/`ToolNode`/
`tools_condition`는 1.0에서 deprecated → `langchain.agents`로 이동. 세밀한 제어가 필요하면 v1
**middleware**로 시스템 프롬프트/스코프 주입·finalize를 처리하고, 그래도 부족하면 커스텀 `StateGraph`.)
LangGraph/LangChain은 **`chat`의 application/infrastructure 레이어에만** 둔다(도메인 순수: 프레임워크
import 금지). 의존(2026.6 기준): `langchain>=1.3,<2`(umbrella; `create_agent` 포함),
`langchain-openai>=1.1,<2`, `langgraph`/`langchain-core`는 transitive(+ Langfuse LangChain 콜백,
`langfuse>=4`). **추출(extraction)은 기존 `generate_json` 어댑터를 그대로 유지** — 챗만 LangChain
chat model 경로를 쓴다(두 경로 공존).

```text
create_agent(model, tools=[6개], system_prompt, ...):
  model ──tool_calls──▶ 도구 실행(toolset) ──ToolMessage──▶ model ⟳(≤recursion_limit)
        └──no tool────▶ 최종 답변 → finalize(출처 매핑 + 영속)
```

- **상한**: `recursion_limit`(= `MAX_STEPS` 설정에서 환산)로 무한 루프 방지. 초과 시 마지막 근거로 best-effort.
- **출처**: 도구 결과가 **이미 `chunk_id`/`cell_id`를 포함**하므로(§2.13 카탈로그), 실행 후 최종
  state의 `ToolMessage`들에서 사용된 청크/셀을 **id로 정확 매핑**해 ChatSource로 저장 — 추출처럼
  quote를 청크에 되맞추는 fuzzy 매칭이 **불필요**(정확도↑). 시스템 프롬프트로 모델이 사용한
  출처를 `[chunk:<id>]`/`[cell:<id>]` 마커로 **명시 인용**하게 하고(등장 순 = rank), 마커가
  없으면 사용된 도구 결과 순 폴백. 메시지당 출처 상한(`MAX_SOURCES`, 기본 5), **도구 결과에
  실재하는 id만 인정**(환각 id 차단). (phase-4 D10)
- **스코프**: 세션 `scope_document_db_id`가 있으면 도구 접근을 그 DB로 제한, 없으면(전역)
  에이전트가 `list_document_dbs`로 직접 고른다.
- **트레이싱**: LangChain용 **Langfuse 콜백 핸들러**로 그래프/도구/LLM 호출을 추적.

#### SSE 스트리밍 (스텝 라이브)

메시지 전송(§6.5)은 **SSE**로 진행을 스트림한다 — 각 도구 호출을 이벤트로 흘려보내
("DB 탐색 중…", "컬럼 확인 중…", "셀 조회 중…") 마지막에 답변+출처를 보낸다. 스텝은
완료 시 `ChatMessage.steps`(§2.10)에 저장 → 세션 재오픈 시 사고 과정 재현.
...
> 비고: 도구는 모두 **읽기 전용**(부작용 없음). MAX_STEPS·도구 결과 토큰 상한·DB 스코프에
> 더해 **동일 (도구,인자) 반복 호출 가드**(2회 초과 시 실행 대신 시정 지시 반환 — 모델
> 루프 차단)가 튜닝/안전 파라미터. 도구 고도화(find_documents/query_cells 필터/리랭커)는
> [phase-5-agent-quality-plan.md](phase-5-agent-quality-plan.md).
...
create table chat_message (
  ...
  steps jsonb,                      -- 에이전트 도구 호출 추적(assistant); §2.13
  created_at timestamptz not null default now()
);
...
create table chat_source (
  ...
  chunk_id uuid references document_chunk(id) on delete set null,  -- 비정형 출처
  cell_id uuid references cell(id) on delete set null,             -- 정형 출처(§2.11)
  quote text not null,
  page int,
  rank int not null default 0,
  created_at timestamptz not null default now(),
  check (chunk_id is not null or cell_id is not null)              -- 둘 중 하나
);
```
...
| GET | `/chat/sessions/{sid}` | 세션 + 메시지(+steps/sources) |
| PATCH | `/chat/sessions/{sid}` | 제목 변경 |
| DELETE | `/chat/sessions/{sid}` | 삭제 |
| POST | `/chat/sessions/{sid}/messages` | 질문 전송 → **에이전트 실행**, **SSE**로 `step`(도구 호출) → `answer`(답변+출처) → `done` 스트림, 실패 시 `error`(§2.13). 세션당 동시 실행 1개(초과 시 409) |
...
12. **리랭커** → ✅ **현재 전 환경에서 재정렬 생략, 벡터 top-n 직행.** `RerankPort`는 추후
   온프렘(BGE-Reranker)에서만 구현(미설정 시 no-op = 벡터 순위 유지).
...
15. **챗 출처 범위** → ✅ **청크 + 셀.** ChatSource는 `chunk_id`(비정형) 또는 `cell_id`(정형) 중
   하나를 인용(§2.11). 도구 결과가 id를 포함하므로 fuzzy quote 매칭 없이 **정확 인용**.
16. **에이전트 스텝 스트리밍** → ✅ **SSE 라이브 스트림** + `ChatMessage.steps` 저장. 진행
   (도구 호출)을 실시간 표시하고 세션 재오픈 시 사고 과정 재현(§2.13, §6.5).
17. **에이전트 오케스트레이션 프레임워크** → ✅ **LangChain v1 `langchain.agents.create_agent`**
   (LangGraph 런타임 위 ReAct) + **native tool-calling**. 구버전 `langgraph.prebuilt`
   (`create_react_agent`/`ToolNode`/`tools_condition`)는 1.0에서 deprecated → `langchain.agents`로
   이동했으므로 신규 표준 API를 쓴다. 세밀 제어가 필요하면 v1 middleware, 그래도 부족하면 커스텀
   `StateGraph`. LangGraph/LangChain은 `chat` application/infra 레이어에만, 도메인은 포트만 의존.
...
운영 LLM E2E 검증 완료(스트리밍 UX·인용 패널·루프 가드까지 운영 반영). 현재 로드맵은
**Phase 5 — 에이전트 품질 고도화**([phase-5-agent-quality-plan.md](phase-5-agent-quality-plan.md),
방향: 챗 우선 + 추출은 얇은 검증 루프):

1. **5-A `find_documents`**(문서명 키워드 검색 도구, D12) + 골든 평가셋(D14).
2. **5-B `query_cells` 서버측 필터/집계**(구조화 필터 DSL, D13).
3. **5-C 하이브리드 검색 + 리랭커**(BGE-Reranker-V2-M3, D15 — §2.12 갱신 동반).
4. **5-D 추출 자기검증 루프**(self-check + 재시도, D16).
5. (후속) 추출 full agent화(검증 병목 실측 시), 읽기전용 그리드 의미검색·폴백 셀
   "부분 컨텍스트" 배지(§2.12), `identity`(인증/권한)·법률 특화 모델(매터/보안등급/ACL —
   §9 오픈 퀘스천), dev LLM 키 발급 시 골든셋 자동 채점 러너.
