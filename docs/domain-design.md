...
#### 에이전트 루프 (LangGraph StateGraph + provider-portable JSON 도구 루프)

**오케스트레이션 = LangGraph 커스텀 `StateGraph`**, **추론(LLM) = `TextGenerationPort.generate_json`**.
즉 LangGraph의 프리빌트 tool-calling 에이전트(`create_react_agent`, 모델 native function-calling
의존)를 **쓰지 않고**, 노드를 직접 구성해 reason 노드에서 JSON 루프를 돌린다 → Gemini(dev)·
온프렘 GLM 양쪽에서 모델의 function-calling 지원 없이 동작(포터블). LangGraph는 상태머신·
스트리밍만 담당하며 **application/infrastructure 레이어에만** 둔다(도메인 순수: `langgraph`/
`langchain-core` import 금지). 의존: `langgraph` + `langchain-core`(경량, PyPI 설치만 — 온프렘 친화).

```text
StateGraph(state = {question, scope, history, tool_results, steps, answer, sources, step_count})
  node reason : generate_json → {"action","args"} | {"answer","sources"}
  node act    : toolset 디스패치(인자 Pydantic 검증) → 결과 누적 + step 기록
  edges       : reason --action--> act --(step<MAX)--> reason ; reason --answer--> finalize
  node finalize: sources→ChatSource 매핑 + 영속
```

- **상한**: `MAX_STEPS`(설정)로 무한 루프 방지. 초과 시 마지막까지의 근거로 best-effort 답변.
- **출처**: 답변의 `sources`를 ChatSource로 매핑 — `kind=chunk`면 `chunk_id`, `kind=cell`이면
  `cell_id`(§2.11). quote→청크 매핑은 추출과 동일 폴백(실패 시 quote만).
...
14. **도구 호출 방식** → ✅ **provider-portable JSON 도구 루프**(`generate_json` 재사용).
   native function-calling(Gemini/vLLM tools) 대신 JSON으로 추상화 → 온프렘 GLM이 function-calling을
   지원하지 않아도 동작. (native는 후속 최적화)
15. **챗 출처 범위** → ✅ **청크 + 셀.** ChatSource는 `chunk_id`(비정형) 또는 `cell_id`(정형) 중
   하나를 인용(§2.11) → 정형 질의의 근거(특정 추출값)도 출처로 연결.
16. **에이전트 스텝 스트리밍** → ✅ **SSE 라이브 스트림** + `ChatMessage.steps` 저장. 진행
   (도구 호출)을 실시간 표시하고 세션 재오픈 시 사고 과정 재현(§2.13, §6.5).
17. **에이전트 오케스트레이션 프레임워크** → ✅ **LangGraph(커스텀 `StateGraph`).** 단,
   프리빌트 tool-calling 에이전트가 아니라 reason 노드가 `generate_json`(JSON 루프, #14)을
   호출 → provider 포터블 유지(온프렘 GLM의 native function-calling 불필요). LangGraph는
   `chat` application/infra 레이어에만 두고 도메인은 포트만 의존. 스트리밍(`astream`)→SSE 스텝.
   의존: `langgraph` + `langchain-core`(경량). 추론이 기존 `generate_json`이라 Langfuse 트레이싱 유지.
