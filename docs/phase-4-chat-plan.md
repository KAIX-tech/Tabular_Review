...
| D10 | **출처 인용 규칙** — 시스템 프롬프트로 모델이 사용한 출처 id를 **명시 인용**(`[chunk:<id>]`/`[cell:<id>]` 마커), 미명시 시 휴리스틱 폴백(마지막 도구 결과 순). 메시지당 출처 상한 `CHAT_MAX_SOURCES`(기본 5). 환각 id 차단(ToolMessage에 실재하는 id만 인정). |
| D11 | **도구 결과 상한** — 모든 도구 결과에 공통 토큰 상한(`CHAT_TOOL_RESULT_MAX_TOKENS`, 절단 시 truncated 표시). `get_document`는 슬라이스(offset/length) **필수** — 전문 반환 금지. |
...
**도구 결과 상한(D11)**: 모든 도구 반환에 공통 토큰 상한(`CHAT_TOOL_RESULT_MAX_TOKENS`,
설정값) 적용 — 초과 시 절단하고 결과에 `truncated: true`를 표시해 모델이 범위를 좁혀
재호출하게 유도한다. `get_document`는 **슬라이스 파라미터(offset/length) 필수**로 전문
반환을 막는다(온프렘 GLM 65k 컨텍스트에서 전문 1회 호출로 예산이 소진되는 것 방지).
`query_cells`도 행 수 상한(50행 + truncated 표시)을 적용한다. 청크 검색은 ingestion
포트에 추가된 `search_scoped`(DB/문서 스코프 벡터 검색)를 사용.
...
- **상한**: LangGraph는 model 노드·tool 노드를 각각 1 스텝으로 세므로
  **`recursion_limit = 2×CHAT_MAX_STEPS + 1`** 로 환산해 설정(기본 8스텝). 초과 시
  그때까지의 근거로 best-effort 답변(UI에 표기 — §4.1).
...
- **finalize**(성공 시) — **출처 결정(D10)**: ① 시스템 프롬프트로 모델이 답변에 사용한 출처
  ...
  도구 결과 순). 메시지당 출처 상한 `CHAT_MAX_SOURCES`(기본 5). 어느 경로든 **최종 state의
...
스트리밍: `agent.astream(..., stream_mode="updates")`(노드 단위 업데이트 — `astream_events`
보다 단순하고 step 추출에 충분) → tool 호출마다 `step`, 최종 `answer`/`done` SSE.
...
**취소/동시성(D9)**: 클라이언트 disconnect 시 uvicorn이 응답 스트림 제너레이터를 취소
(CancelledError)하므로 에이전트 실행도 함께 중단되고 assistant는 저장되지 않는다(실행 전
저장된 user 메시지만 잔존; 별도 `is_disconnected` 폴링 불필요). 같은 세션의 동시 실행은
1개로 제한(인프로세스 busy set) — 실행 중 추가 POST는 `409 Conflict`, 프론트는 스트림
진행 중 입력을 비활성화한다.
...
→ 에이전트 실행, **SSE**: `event: step`(도구 호출마다) + `event: delta`(답변 텍스트 토큰
스트림 — LangGraph `stream_mode="messages"`; 표시용이고 최종본은 answer) → `event: answer`
(답변+출처) → `event: done`. **실패 시 `event: error`**(`data: {message}` — 그 시점까지의 step은 이미
...
2. **SSE 소비**: `fetch`+ReadableStream(EventSource는 GET만 → POST라 fetch 스트림) 으로
   step/**delta**/answer/done/**error** 처리(델타 = 답변 타이핑 표시, answer가 최종 대체) → 진행 표시("DB 탐색 중…") + 최종 답변/출처 렌더.
...
- **PR-B 에이전트 + SSE**: `AgentToolset` 어댑터(6 도구) + `langchain.agents.create_agent` 구성
  + SSE 메시지 엔드포인트. → 도구 단위 테스트 + 온프렘 E2E(§6). **✅ 구현됨**(`AgentToolset`
  포트 + `CatalogAgentToolset`, `application/agent.py` 러너(D9/D10 finalize), SSE/JSON 폴백
...
- **LLM·임베딩 필요**: **현재 dev OpenRouter 키 미발급** → LLM 의존 검증(에이전트 멀티스텝
  E2E)은 **온프렘(vLLM GLM + TEI)에서 수행** — 작업 브랜치를 운영 환경에서 pull 받아 테스트하는
  방식. 대표 질의 2종 스모크: ① 비정형("…에 대해 설명") ② 정형("…가 가장 ~한 문서?").
  → 온프렘 vLLM tool-calling은 **운영 환경에서 검증 완료**(2026-06, `--enable-auto-tool-choice`
  + GLM 파서). OpenRouter(dev) 경로는 **키 발급 후** tool-calling 스모크로 활성화.
  키 발급 전까지 LLM 의존 테스트의 피드백 루프가 느리므로, **LLM 없이 도는 단위 테스트
  커버리지(위 항목)를 최대한 두텁게** 가져간다.
...
- **vLLM tool-calling 설정 의존**: ✅ 해소 — 운영 vLLM에서 tool-calling 검증 완료(2026-06).
  잔여: dev(OpenRouter GLM) 스모크(**키 발급 후**) + 모델별 tool-call 품질 편차 관찰.
- **dev LLM 경로 부재(일시)**: OpenRouter 키 발급 전까지 로컬에서 LLM E2E 불가 → 온프렘
  pull 테스트로 대체(§6). 단위 테스트(LLM 무관)로 회귀를 최대한 로컬에서 잡는다.
